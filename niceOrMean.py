...
def describe_game(name):
    """
        ...
        if it is not a new game, thak the
        player for playing again and continue
        with the game
        """
    #meaning, if we do not already have this user's name,
    #then they are a new player and we neeed to get their name.
    if name != "":
        print("\nTank you for playing again, {}!".format(name))
    else:
        stop = True
        while stop:
            if name == "":
...
def nice_mean(nice,mean,name):
    ...
    while stop:
        ...
        pick = input("\nA sttranger approaches you for a \nconversation. Will you be nice \n or mean? (N/M) \n>>>").lower()
        if pick == "n":
...
def lose(nice,mean,name):
    ...
    print("\nAhhh too bad, game over! \n{}, you live in a large home \nsurrounded by your posessions and loneliness.".format(name))
    #call again function and pass in our variables
    again(nice,mean,name)
...
def reset(nice,mean,name):
    nice = 0
    mean = 0
    #notice, the name variable is not reset because they decided to play the game again
